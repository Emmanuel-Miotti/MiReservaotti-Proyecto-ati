  ...
  const fetchReservas = async () => {
    try {
      ...
      const [response1, response2] = await Promise.all([
        axios.get(`http://127.0.0.1:8000/api/v1/reservas/empresa/${userId}`),
        axios.get(`http://127.0.0.1:8000/api/v1/reservas2/empresa/${userId}`),
      ]);
    ...
    try {
      if (editMode) {
        const url = formData.isRegistered
          ? `http://127.0.0.1:8000/api/v1/reservas/${formData.id}`
          : `http://127.0.0.1:8000/api/v1/reservasUsuarioNoRegistrado/${formData.id}`;
  ...
  const handleDelete = async (id, isRegistered) => {
    try {
      const url = isRegistered
        ...
        : `http://127.0.0.1:8000/api/v1/reservasUsuarioNoRegistrado/${id}`;
