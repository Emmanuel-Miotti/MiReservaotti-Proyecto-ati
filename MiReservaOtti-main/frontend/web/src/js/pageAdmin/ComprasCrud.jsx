...
const CompraCRUD = () => {
  ...
  const fetchCompras = async () => {
    try {
      ...
      console.log(userId);
      const response = await axios.get(`http://127.0.0.1:8000/api/v1/empresa/compras/${userId}`);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const url = `http://127.0.0.1:8000/api/v1/compras/${formData.id}`;
    try {
      if (editMode) {
        await axios.put(url, formData);
      } else {
        await axios.post('http://127.0.0.1:8000/api/v1/compras', formData);
      }
      fetchCompras();
      setShowModal(false);
      resetFormData();
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://127.0.0.1:8000/api/v1/compras/${id}`);
      fetchCompras();
    } catch (error) {
