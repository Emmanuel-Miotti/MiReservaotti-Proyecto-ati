...
const AgendaPanel = () => {
  ...
  const obtenerReservas = async () => {
    try {
      ...
      console.log(reservasNoRegistradas)
      console.log(reservasRegistradas)
  ...
  const eventos = reservas.map((reserva) => ({
    id: reserva.id,
    // title: `${reserva.isRegistered ? reserva.cliente_id : reserva.nombre_cliente}`,
    title: `${reserva.isRegistered ? reserva.cliente.name : reserva.nombre_cliente}`,
      ...
      <Container className="mt-1">
        ...
        <Modal show={eventoSeleccionado !== null} onHide={handleCloseModal}>
          ...
          <Modal.Body>
            {eventoSeleccionado && (
              <>
                <p><strong>Cliente:</strong> {eventoSeleccionado.title}</p>
                <p><strong>Servicios:</strong> {eventoSeleccionado.servicios.map(servicio => servicio.nombre).join(', ')}</p>
                <p><strong>Fecha:</strong> {moment(eventoSeleccionado.start).format('LL')}</p>
                <p><strong>Hora de Inicio:</strong> {moment(eventoSeleccionado.start).format('LT')}</p>
                <p><strong>Hora de Fin:</strong> {moment(eventoSeleccionado.end).format('LT')}</p>
                <p><strong>Duración:</strong> {moment.duration(moment(eventoSeleccionado.end).diff(moment(eventoSeleccionado.start))).asMinutes()} minutos</p>
