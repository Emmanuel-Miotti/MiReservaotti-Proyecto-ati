...
const ProductosCompra = () => {
    ...
    const fetchProductos = async () => {
        ...
        try {
            const response = await Config.getProductosActivosEmpresa(empresaId); // Ajusta según tu configuración real
    ...
    const confirmarCompra = () => {
        if (Object.keys(carrito).length === 0) {
            setError2("Debes seleccionar al menos un producto antes de confirmar la compra.");
            return;
        }
    ...
    const realizarCompra = async () => {
        ...
        // import Config from "../Config";
        const response = `${Config.url()}/compras`; // URL de la API
        if (response.error) {
            console.log("eeeee")
        } 
        ...
        try {
            ...
            setError2(error.message || "Error desconocido al realizar la compra.");
    ...
    const irAlHome = async () => {
        navigate(`/Cliente`);
    }


    const volverEmpresa = async () => {
        ...
        <Container>
            <Row>
                ...
                <Col md={4}>
                    ...
                    {error2 && <p className="text-danger">{error2}</p>} {/* Muestra el error en rojo */}
    ...
    <td>${Object.values(carrito).reduce((acc, item) => acc + item.subtotal, 0).toFixed(0)}</td>
</tr>
                        </tbody>
                    </Table>
                    <Button variant="success" onClick={confirmarCompra}>Pagar en persona</Button>
            ...
            <Modal show={!!mensaje || !!error} onHide={() => {
                ...
                <Modal.Footer>
                    ...
                    <Button variant="secondary" onClick={(volverEmpresa)}>
                        Volver a al empresa 
                    </Button>

                    <Button variant="secondary" onClick={(irAlHome)}>
                        Ir al home 
                    </Button>
